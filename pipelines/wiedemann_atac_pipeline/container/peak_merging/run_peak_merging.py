...
print("Sorting broad peaks...", flush=True)
subprocess.run(
...
print("Merging peaks...", flush=True)
subprocess.run(
    f"Gipfelkreuzer -l WARN -b 6 -o {narrow_merged_path} {narrow_combined_path}",
    cwd=MOUNT_PATHS["output"],
    shell=True,
    check=True,
)
...
copy_into(in_file=narrow_merged_path, out_file=merged_path)
copy_into(in_file=broad_merged_path, out_file=merged_path)

...
for root, dirs, files in os.walk(INPUT_FOLDER):
    for file in files:
        if file.endswith(BED_SUFFIX):
            print(f"Processing file {os.path.join(root, file)}...")
            file_base_name = file.removesuffix(BED_SUFFIX)
            ...
            subprocess.run(
                (
                    f"bedtools intersect -a {file_base_path}{BED_SUFFIX} -b {BLACKLIST_PATH} -wa -v "
                    f"> {file_base_output_path}{BED_SUFFIX}"
                ),
                shell=True,
                check=True,
            )
            ...
            subprocess.run(
                (
                    f"bedtools bedtobam -i {file_base_output_path}{BED_SUFFIX} -g {CHROM_SIZES_PATH} "
